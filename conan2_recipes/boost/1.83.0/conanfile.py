...
# This recipe installs the headers of the selected Boost libraries currently needed by GammaRay (not the entirety of them).
# ATTENTION: if making changes to the library selection, it is also necessary to run the accompaining `rebuild-dependencies.py` script
#            with the same library list as set in CONFIGURE_OPTIONS list.  Also check comments throughout the recipe containing "GammaRay"
#            as such code was commented-out due to missing items in CONFIGURE_OPTIONS list.
# ATTENTION 2: The `header_only` option is enabled, meaning that no library will be built.  Currently, GammaRay uses only Boost headers.
#              So, if you decide to reuse this recipe to build something else, please, pay attention to this detail.

# Here are only the Boost libraries that GammaRay currently uses.
# The complete list are the directory names in /boost source subdirectory containing .hpp files.
# When adding (or removing) an option, also add/remove the option to the list in
# `rebuild-dependencies.py` and re-run that script.
CONFIGURE_OPTIONS = (
    "geometry",
    "numeric"
)
...
class BoostConan(ConanFile):
    ...
    # default_options.update({f"without_{_name}": True for _name in ("graph_parallel", "mpi", "python")})  --> None of these Boost libraries are used by GammaRay
    ...
    def configure(self):
        ...
        if self.options.i18n_backend != "deprecated":
            ...
            if self.options.i18n_backend == "None":
                self.options.i18n_backend_iconv = "off"
                self.options.i18n_backend_icu = False
        if True: #if self.options.without_locale:                   --> boost::locale is not used by GammaRay
            self.options.rm_safe("i18n_backend_iconv")
            self.options.rm_safe("i18n_backend_icu")

        #if not self.options.without_python:                        --> boost::python is not used by GammaRay
        ...
        if True: #if self.options.without_fiber:                   --> boost::fiber is not used by GammaRay
    ...
    def validate(self):
        ...
        #if not self.options.without_locale and self.options.i18n_backend_iconv == "off" and \                    --> boost::locale is not used by GammaRay
