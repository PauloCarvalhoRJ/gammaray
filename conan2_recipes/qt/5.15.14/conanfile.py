...
class QtConan(ConanFile):
    ...
    default_options = {
        ...
        "with_gssapi": False,
        "with_atspi": False,
        "with_md4c": True,  #--> Qt Quick needs QTextDocument::setMarkdown() which only exists if MarkdownReader module is enbled in QtGui's text formats.
        "with_x11": True,
        "gui": True,
        "widgets": True,
        "android_sdk": None,
    ...
    def config_options(self):
        ...
        if self.settings.os == "Windows":
            if self.settings.compiler == "msvc":
                self.options.with_doubleconversion = True #self.settings.compiler.libcxx is not available with Visual Studio, thus, double-conversion is needed.
    ...
    def build_requirements(self):
        if self._settings_build.os == "Windows" and is_msvc(self):
            #self.tool_requires("jom/1.1.3") --> recipe's original
            self.tool_requires("jom_installer/1.1.2")
        if self.options.qtwebengine:
