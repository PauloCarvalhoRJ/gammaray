...
from conan.tools.files import ( apply_conandata_patches, copy, export_conandata_patches, 
                                get, replace_in_file, rmdir, save, unzip )
...
# This recipe builds, installs and tests the ITK library (The Insight Toolkit).
...
class ITKConan(ConanFile):
    ...
    # Conan's workspace.
    # ODDITY: For some reason, decompressing the sources archive in source() callback causes an odd 
    #        "The CMake policy CMP0091 must be NEW, but is ''" error...
    #        so, this recipe uses a different unusual procedure to decompress the source code (see the 
    #        export_sources() callback.
    # exports_sources = "itk-{}.tar.gz".format(version)
    ...
    def export_sources(self):
        ###########################################################################################
        #  Decompressing the source archive is normally done in the source() callback, but doing
        #  so there, an odd "The CMake policy CMP0091 must be NEW, but is ''" error takes place.
        print("Deflating the archive with the source files into the export sources directory...")
        unzip(self, filename=os.path.join(self.recipe_folder, "itk-{}.tar.gz".format(self.version)), \
    ...
    def requirements(self):
        ...
        # self.requires("expat/2.5.0") --> from the original recipe (conflicts with gdcm's dependency on expat 2.6.2)
        self.requires("expat/2.6.2") # --> to agree with gdcm's own dependency on expat
        self.requires("fftw/3.3.10")
        self.requires("gdcm/3.0.21")
        self.requires("hdf5/1.14.1")
        self.requires("libjpeg/9e")
        # self.requires("libpng/1.6.40") --> from the original recipe
        self.requires("libpng/1.6.42")
        self.requires("libtiff/4.5.1")
        # self.requires("openjpeg/2.5.0") --> from the original recipe
    ...
    def source(self):
        ...
        #unzip(self, filename=os.path.join(self.source_folder, "..", "itk-{}.tar.gz".format(self.version)), destination=".")
        #copy(self, pattern="*", src=os.path.join(self.source_folder, "itk-{}".format(self.version)), dst=".")
        # ODDITY: for some reason, decompressing the source archive here causes an odd "The CMake policy CMP0091 must be NEW, but is ''" error...
        #         so, it is unzipped earlier in the export_sources() callback and only the copy part is done here.  This is not the usual procedure as it
        #         increases both build time and disk space used by the package (too many copies of the uncompressed source files).
        copy(self, pattern="*", src=os.path.join(self.source_folder, "..", "itk-{}".format(self.version)), dst=".")
    ...
    def package(self):
        ...
        for cmake_file in glob.glob(os.path.join(self.package_folder, self._cmake_module_dir, "*.cmake")):
            if os.path.basename(cmake_file) != "UseITK.cmake" and \
               os.path.basename(cmake_file) != "ITKInitializeCXXStandard.cmake" and \
               os.path.basename(cmake_file) != "ITKFactoryRegistration.cmake":
    ...
    def package_info(self):
        ...
        # ATTENTION: Consumer packages configured with CMake need to feed this value into a CMake cache variable called
        # ITK_CMAKE_DIR in order to UseITK.cmake to work: 
        # 
        #    def generate(self):
        #        tc = CMakeToolchain(self)
        #        itk_cmake_dir = self.dependencies[self.tested_reference_str].cpp_info.get_property("itk_cmake_dir")
        #        tc.cache_variables["ITK_CMAKE_DIR"] = itk_cmake_dir.replace("\\", "/") # backslashes in Windows paths result in syntax errors in CMake
        #        tc.generate()
        #        tc = CMakeDeps(self)
        #        tc.generate()
        #
        itk_cmake_dir = os.path.join(self.package_folder, "lib", "cmake", self._itk_subdir )
        self.cpp_info.set_property("itk_cmake_dir", itk_cmake_dir)
