...
void VecticalProportionCurvesCanvasPicker::move( const QPoint &pos )
{
        ...
        i < static_cast<int>( d_selectedCurve->dataSize() ); i++ )
    {
        ...
        {
            //move the curve points only left and right
...
//                d_selectedCurve->yAxis(), pos.y() );
        ...
        //limits horizontal movement to a range between 0.0 and 100.0,
        //since there is no valid proportion value outside that range
        if( xData[i] > 100.0)
            xData[i] = 100.0;
        if( xData[i] < 0.0)
            xData[i] = 0.0;
