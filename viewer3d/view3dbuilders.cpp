...
vtkSmartPointer<vtkProp> View3DBuilders::build(Attribute *object)
{
    ...
    if( fileType == "POINTSET" ){
        ...
        if( cg->getNZ() < 2 ){
            ...
            return buildForAttributeCartesianGrid( cg, attribute);
        }
    } else {
...
vtkSmartPointer<vtkProp> View3DBuilders::buildForAttributeCartesianGrid(CartesianGrid *cartesianGrid, Attribute *attribute)
{
    //load grid data
    cartesianGrid->loadData();
